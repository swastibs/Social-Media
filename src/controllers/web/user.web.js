...
exports.toggleFollow = async (req, res, next) => {
    try {
        ...
        const existing = await UserFollow.findOne({ where: { followerId, followingId } });

        if (existing) {
            await existing.destroy();
            await User.decrement("followingCount", { by: 1, where: { id: followerId } });
            await User.decrement("followersCount", { by: 1, where: { id: followingId } });
