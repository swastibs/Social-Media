const { User, Post, UserFollow, PostLike, Comment, sequelize } = require("../../models");
...
const getFeedPosts = async (currentUserId, limit, offset, followingIds) => {
    ...
    const commentMap = Object.fromEntries(commentCounts.map(cc => [cc.postId, parseInt(cc.count)]));
...
exports.renderFeed = async (req, res, next) => {
    try {
        ...
        const userWithCounts = await User.findByPk(currentUser.id, {
            attributes: ["id", "name", "email", "bio", "profilePictureUrl", "postsCount", "followersCount", "followingCount"],
        });
